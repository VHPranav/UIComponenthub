...
// Generic delay helper if not natively exported from older framer-motion versions
...
/**
 * RadialIntro Component
 * Creates an orbital animation for a set of images.
 * Features a sequential "lift-in" entrance followed by a continuous orbital spin.
 */
export function RadialIntro({
  ...
  }, [animate, scope, orbitItems.length]);
      ...
      >
        {orbitItems.map((item, i) => (
          ...
          >
            <motion.img
              data-arm-image
              className="rounded-full object-fill absolute left-1/2 top-1/2 aspect-square translate -translate-x-1/2 overflow-hidden border-2 border-white dark:border-zinc-950 shadow-md"
              style={{
                width: imageSize,
                height: imageSize,
                opacity: i === 0 ? 1 : 0,
                top: "50%", // Initial position for the lift-in animation
              }}
              src={item.src}
              alt={item.name}
              draggable={false}
